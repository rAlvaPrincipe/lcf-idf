...
def a_512(seed, is_small):
    tokenizer = AutoTokenizer.from_pretrained("bert-base-uncased", do_lowercase=True)   
    
    ids, texts, labels = [], [], []
    for line in open("data/amazon_electronics/reviews_Electronics_5.json"):
        diz = json.loads(line)
    ...
    df['is_long'] = df['n_subwords'].apply(lambda x: True if x > 512 else False)
...
def a_512_small(seed):
    out = ("a_512_small", ) + a_512( seed, True)
    return out
